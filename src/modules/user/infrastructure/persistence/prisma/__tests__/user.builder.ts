...
export class UserBuilder {
  ...
  build(): User {
    const defaultProps: UserProps = {
      email: this.props.email || Email.create("default.user@example.com").getValue(),
      password: this.props.password || Password.create("DefaultPass123!").getValue(),
      isEmailVerified: this.props.isEmailVerified === undefined ? false : this.props.isEmailVerified,
    };

    return User.create(defaultProps, this.id).getValue();
  }
}
