...
import { UserBuilder } from "../../../../user/infrastructure/persistence/prisma/__tests__/user.builder";
...
describe("RegisterUserUseCase (Sociable Unit Test)", () => {
  ...
  beforeEach(() => {
    ...
    useCase = new RegisterUseCase(userRepository, emailService, roleRepository);
  ...
  describe("execute", () => {
    ...
    it("should return error when roleId does not exist in the roles table", async () => {
      ...
      expect(errorMessage).toBe("Invalid role");
      expect(userRepository.save).not.toHaveBeenCalled();
      expect(emailService.sendConfirmationEmail).not.toHaveBeenCalled();
    });
  });
